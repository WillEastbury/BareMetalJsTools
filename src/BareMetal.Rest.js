...
BareMetal.Rest = (() => {
  ...
  function _maybeCompress(body, headers) {
    if (!_hasCompression()) return body;
    let bytes;
  ...
  async function call(method, url, body) {
    ...
    if (body !== undefined) {
      if (body instanceof FormData) {
        ...
        opts.body = _maybeCompress(json, opts.headers);
      }
    }
    _addAcceptEncoding(opts.headers);
  ...
  async function binaryCall(method, url, body) {
    ...
    if (body instanceof ArrayBuffer || body instanceof Uint8Array) {
      ...
      opts.body = _maybeCompress(body, opts.headers);
    }
    _addAcceptEncoding(opts.headers);
    if (method !== 'GET' && method !== 'HEAD') {
      opts.headers['X-Requested-With'] = 'BareMetalWeb';
